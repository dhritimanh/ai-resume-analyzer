...
import { QuickAnalysisFreeResult, getScoreColor, getScoreLabel } from '@/lib/resume-analysis/quick-analysis-free';

interface QuickAnalysisFreeProps {
  resumeContent: string;
  onUpgrade?: () => void;
}

export default function QuickAnalysisFree({ resumeContent, onUpgrade }: QuickAnalysisFreeProps) {
  const [analysis, setAnalysis] = useState<QuickAnalysisFreeResult | null>(null);
  ...
  const analyzeResume = async () => {
    ...
    try {
      const response = await fetch('/api/analysis/quick-free', {
        method: 'POST',
      ...
      console.error('Quick analysis free error:', err);
  ...
  return (
    <div className="bg-[#1a1a1a] rounded-xl border border-[#2a2a2a] overflow-hidden">
      ...
      {/* Quick Wins - FREE TIER (2-3 insights) */}
      <div className="p-6 border-b border-[#2a2a2a]">
        <div className="flex items-center justify-between mb-3">
          <h4 className="text-sm font-semibold text-white">⚡ Quick Wins</h4>
          <span className="text-xs text-gray-500">{analysis.quickWins.length} insights</span>
        </div>
        <div className="space-y-3">
        ...
        {onUpgrade && (
          <div className="mt-4 bg-gradient-to-r from-purple-500/10 to-pink-500/10 border border-purple-500/30 rounded-lg p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-normal text-white mb-1">🔒 Want the detailed section-by-section breakdown?</p>
                <p className="text-xs text-gray-400">Get 50+ More Insights with specific rewrites</p>
              </div>
              <button
                onClick={onUpgrade}
                className="bg-gradient-to-r from-purple-600 to-pink-600 text-white px-4 py-2 rounded-lg text-xs font-semibold hover:from-purple-700 hover:to-pink-700 transition-all whitespace-nowrap"
              >
                Unlock
              </button>
            </div>
